...
def search_news(company, date):
    ...
    for result in json_results['d']['results']:
        ...
        if d > date:
            url = result['Url']
            title = result['Title'].replace(u"\u2018", '').replace(u"\u2019", '').replace(u'\u20ac', '').replace(u'\u201c', '').replace(u'\u201d', '').replace(u'\u2013', '').replace('\'', '').replace('"', '')
            desc = result['Description'].replace(u"\u2018", '').replace(u"\u2019", '').replace(u'\u20ac', '').replace(u'\u201c', '').replace(u'\u201d', '').replace(u'\u2013', '').replace('\'', '').replace('"', '')
